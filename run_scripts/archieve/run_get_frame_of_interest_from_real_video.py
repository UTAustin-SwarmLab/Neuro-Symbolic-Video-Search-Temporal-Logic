...
if __name__ == "__main__":
    ...
    frame2automaton = VideotoAutomaton(
        ...
        artifact_dir=config.VERSION_AND_PATH.ARTIFACTS_PATH,
        proposition_set=["dog", "store"],
        is_annotation=True,  # TODO: Debug only
        save_image=True,  # TODO: Debug only
        ltl_formula='P>=0.80 ["dog" U "store"]',  # 'P>=0.99 [F "person"]' P>=0.99 [F ("person" U "car")] P>=0.99 [F "person" U "car"]
    )

    frame_of_interest_obj = frame2automaton.run()
    frame_of_interest_obj.save_frames("/opt/Neuro-Symbolic-Video-Frame-Search/artifacts/tmp")
