...
multiprocessing.set_start_method("spawn", force=True)  # Add this line to your code
...
benchmark_frame_video_root_dir = Path(
    "/opt/Neuro-Symbolic-Video-Frame-Search/store/nsvs_artifact/_validated_benchmark_video_20_samples"
)
...
############### Variable
root_path = Path("/opt/Neuro-Symbolic-Video-Frame-Search/store/nsvs_artifact/experiment_2.1_nsvs_ltl/clip")
...
mapping_param_k = 50
save_csv_file_name = get_file_or_dir_with_datetime(base_name="clip_benchmark_search_result", ext=".csv")
...
def run_search(false_threshold=0.10):
    ...
    save_csv_file_name = get_file_or_dir_with_datetime(
        base_name="clip_benchmark_search_result_multithread", ext=".csv"
    )
    ...
    for image_set in benchmark_image_set_dir:
        ...
        for ltl_spec in ltl_video_dir_set:
            ...
            for benchmark_video_file in benchmark_video_file_list:
                ...
                cv_detection_model = ClipPerception(config=config, weight_path=None)
                csv_result["cv_model"] = "clip"
                csv_result["cv_model_weight"] = "clip"
                ...
                # store results
                get_file_or_dir_with_datetime(f"result_{ltl_formula}_{benchmark_video_file.name}", ".pkl")
...
if __name__ == "__main__":
    ...
    inputs = [x / 100 for x in range(start, end + step, step)]
    # inputs = []
