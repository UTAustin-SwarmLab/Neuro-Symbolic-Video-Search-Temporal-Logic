...
from ns_vfs.model_checking.proposition import process_proposition_set
from ns_vfs.model_checking.video_state import VideoState
        ...
        if self.previous_states:
            for prev_state in self.previous_states:
                ...
                for cur_state in current_states:
                    ...
                    self.transition_map[prev_state.state_index].append(
                        cur_state.state_index
                    )
    ...
    def _get_probability_of_propositions(self, frame: VideoFrame) -> None:
        ...
        for i, prop in enumerate(self.proposition_set):
            if frame.object_of_interest.get(prop):
                ...
                prop = prop.replace("_", " ")
                if frame.object_of_interest.get(prop):
                    probability = frame.object_of_interest[
                        prop
                    ].get_probability()
                else:
                    probability = 0.0
