...
class BenchmarkVideoFrameProcessor(VideoFrameProcessor):
    ...
    def __init__(
        ...
        if manual_confidence_probability is None:
            ...
            try:
                self._manual_confidence_probability = float(
    ...
    def process_and_get_frame_of_interest(
        ...
        frame_set, interim_confidence_set = self._reset_frame_set_and_confidence(
            proposition_set
        )
        ...
        for idx in range(total_num_frames):
            ...
            if len(frame_set) > 0:  # propositions in frame
                ...
                if result:
                    ...
                    ) = self._reset_frame_set_and_confidence(proposition_set)
            frame_idx += 1
    ...
    def get_frame_of_interest(
            ...
            if proposition_condition > 0:
                ...
                if result:
                    ...
                    if len(temp_frame_set) > 1:
                        frame_of_interest.append(
                            [frame.frame_index for frame in temp_frame_set]
                        )
                    else:
        ...
        if reverse_search:
            ...
            frame_of_interest = [
                x for x in range(total_num_frames) if x not in flattened_list
            ]
