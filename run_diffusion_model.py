...
if __name__ == "__main__":
    ...
    for key, value in loaded_data.items():
        if value.verification_result == "true":
            ...
            i += 1
            # for frame in value.frame_image_set:
            #     # print(i)
            #     # plt.imshow(frame.frame_image)
            #     # plt.savefig(f"/opt/Neuro-Symbolic-Video-Frame-Search/frame_{i}.png")
            #     dif_img = pix_2_pix.diffuse(frame.frame_image)
            #     plt.imshow(dif_img)
            #     plt.savefig(f"/opt/Neuro-Symbolic-Video-Frame-Search/dif_img_{i}.png")
            #     i += 1
